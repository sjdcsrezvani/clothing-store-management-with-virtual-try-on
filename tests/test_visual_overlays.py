...
def test_overlay_surfaces_are_seen_in_every_palette(probe_server, theme_id):
    evidence, offenders = _overlay_cell(probe_server, theme_id)
    return evidence, offenders


def _overlay_cell(probe_server, theme_id: str) -> tuple[list[str], list[str]]:
    """One palette's overlays at the pixels: lightbox, popup and drawer, each
    composited against its declared scrim and shadow tokens."""
    _seed_saved_tryon()
