...
def upgrade(engine, target: int = MIGRATION_VERSION) -> int:
    ...
    if current < 3 <= target:
        with engine.begin() as conn:
            conn.execute(text("UPDATE schema_version SET version=3 WHERE id=1"))
    if current < 4 <= target:
        with engine.begin() as conn:
            conn.execute(text("UPDATE schema_version SET version=4 WHERE id=1"))
    if current < 5 <= target:
        with engine.begin() as conn:
            conn.execute(text("UPDATE schema_version SET version=5 WHERE id=1"))
