...
- **One label colour for every filled surface could not work.** White on Midnight's brand fill read 2.66:1 and on its mint 2.12:1, because every fill wore a single shared label. Each family now chooses its own: `--brand-label` for the candy fills, `--success-label` for the mint ones — the pole (white or black) that reads on the *worse* end of its own gradient, so a label that passes on one end while the other fails unseen is ruled out by construction. The fill keeps the palette's hue unless no pole reads on it as it came (two fills moved, one step each). Midnight's brand now reads 5.70:1 black-on-candy and its mint 6.93:1; a shop's custom brand re-derives both pairs from its two colours.
- **The loyalty metals were frozen hexes doing the same thing.** `.tier-diamond` painted a white label on `#A66CFF` — 3.3:1, the same latent bug — and the silver and gold pairs were hand-tuned for the light palettes. Each metal is a derived pair now (`tier_pair`): black labels light metal in every palette, the fill honours the frozen stop where that label already reads on it and walks darker where it does not — the diamond's dark end moved to `#7364E8` in most palettes and `#8662D7` in Kids Boutique, whose lavender is lighter — and the gold and diamond ends follow the palette's own sunshine and lavender, so a palette that moves its hue moves its metals.
- **Paper became a palette of its own.** The print rules hard-coded a white-and-black document; `--paper`/`--paper-ink` say it once per theme, so a palette owns even its printer, the discount line carries its emphasis in weight rather than a green chosen for one palette, and the printed border is measured against the paper it prints on. `--accent-ink` gives the lavender a voice as text, which it never had.
- **The stylesheet declares no colour the theme should own.** The sweep took out two `#E8DDD5` borders that vanished on the dark palettes, a dead `.nav-btn`, badge and warning pairs that hand-tuned what the theme already derives, the danger zone's frozen red next to palettes whose danger is derived, and ten theme-block stops that shadowed tokens with the same hexes. The guard cuts the one `:root` block away and fails on any literal left outside it — not fooled by `var(--tier-silver-label)` matching the named colour *silver*, nor by a hex hidden inside a `var()` call — asserts the block it cuts is exactly one, and the floors measure the metals and the accent ink in all ten palettes, with an inventory so the floors table cannot be weakened silently. Every rule was sabotaged and caught before it was trusted.

### گوشی ضبط عکس: a phone is set up by scanning, not by typing

- **The capture page used to be configured by hand.** Whoever held the phone typed the computer's LAN address into one field and the app's static API token into another — a shared, never-rotated secret copied off the server's environment, and an address that was a typo waiting to happen, on the one screen used standing up, holding a child's photo. The SMS gateway's phone had already solved both halves, so this is that pattern: pairing issues a per-device key stored only as its SHA-256, shown exactly once in a QR that carries the address and key together; pairing again kills the old key, which is also how a lost phone is shut out.
- **The credential never rides in a request line.** The QR puts the key in the URL fragment, which browsers never transmit; the pairing landing page is session-free (the phone may not be logged in when it scans), stores the key locally and proves it with a ping. Uploads authenticate by `X-Capture-Key` — distinct from the gateway's header on purpose: two phones, two credentials. The static token no longer opens the capture routes, and the browser session still does. The pairing card lives on the try-on page, owner-only; fourteen tests cover the hash-only storage, the QR shown once, the gate, the unpair and the role, each sabotaged and caught.

**No schema change.** The database stays on revision 19 (the capture phone's key hash lives in the settings store).
