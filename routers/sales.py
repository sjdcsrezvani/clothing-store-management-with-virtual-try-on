...
from services.checkout import (
    create_checkout, get_checkout, update_checkout_options, reserve_basket, finalize_basket,
    set_payment_pending, set_payment_outcome, complete_checkout,
    atomic_decrement_stock, InsufficientStockError,
)
...
async def sales_send_to_terminal(
    ...
    if checkout.state in {"completed", "expired", "payment_cancelled", "payment_declined"}:
        raise HTTPException(status_code=409, detail="این سبد دیگر قابل پرداخت نیست.")
    if checkout.state in {"payment_pending", "payment_approved", "payment_uncertain"}:
        existing = db.query(POSTransaction).filter(POSTransaction.checkout_nonce == checkout.checkout_nonce).first()
        if existing:
            return _reuse_pos_transaction(request, db, existing, int(checkout.final_amount))
        checkout_data = {
            "basket": json.loads(checkout.basket_json or "[]"),
            "final_amount": checkout.final_amount,
        }
    else:
        try:
            checkout_data = finalize_basket(db, checkout)
    ...
    snapshot = checkout.basket_json if len(checkout.basket_json or "") <= 10000 else checkout.basket_json[:10000]
...
async def sales_confirm(
    ...
    if not checkout:
        ...
        checkout_nonce = checkout.checkout_nonce
        request.session["checkout_nonce"] = checkout_nonce
    if checkout.staff_user_id != request.session.get("staff_user_id"):
        raise HTTPException(status_code=404, detail="سبد خرید یافت نشد.")
    if checkout.state in {"completed", "expired", "payment_cancelled", "payment_declined", "refunded"}:
        raise HTTPException(status_code=409, detail="این سبد دیگر قابل تکمیل نیست.")
    if checkout.customer_id != (customer_id or None):
        raise HTTPException(status_code=409, detail="مشتری سبد تغییر کرده است؛ لطفاً سبد جدیدی بسازید.")
    customer = _resolve_customer(checkout.customer_id or 0, db)

    if checkout.state in {"payment_pending", "payment_approved", "payment_uncertain"}:
        if payment_method != checkout.payment_method:
            raise HTTPException(status_code=409, detail="روش پرداخت پس از شروع پرداخت قابل تغییر نیست.")
        if checkout.state == "payment_approved":
            submitted_basket = json.loads(basket_json or "[]")
            stored_basket = json.loads(checkout.basket_json or "[]")
            submitted_ids = [(int(i.get("variant_id")), int(i.get("quantity") or 1)) for i in submitted_basket]
            stored_ids = [(int(i.get("variant_id")), int(i.get("quantity") or 1)) for i in stored_basket]
            if submitted_ids != stored_ids:
                raise HTTPException(status_code=409, detail="سبد پس از شروع پرداخت قابل تغییر نیست.")
    else:
        update_checkout_options(
            db, checkout,
    ...
    try:
        if checkout.state in {"payment_pending", "payment_approved"}:
            checkout_data = {
                ...
                "discounts": {"total_discount": checkout.discount_amount, "details": []},
                "credit_surcharge": checkout.credit_surcharge,
                "final_amount": checkout.final_amount,
            }
        else:
            checkout_data = finalize_basket(db, checkout)
    ...
    referrer = _resolve_referrer(referrer_code, referrer_phone, db) if customer else None
    _grant_referred_discount(referrer, customer, db) if customer else None
    total_amount = checkout_data["total_amount"]
    discounts = checkout_data["discounts"]
    credit_surcharge = checkout_data["credit_surcharge"]
    final_amount = checkout_data["final_amount"]
...
async def sale_refund(sale_id: int, request: Request, refund_reason: str = Form(""), db: Session = Depends(get_db)):
    ...
    # Preserve the original sale and create an immutable refund ledger entry.
    from services.ledger import create_refund
    existing_refund = db.query(Refund).filter(Refund.sale_id == sale.id).first()
    if not existing_refund:
        refund_lines = [
            {"sale_item_id": item.id, "quantity": item.quantity,
