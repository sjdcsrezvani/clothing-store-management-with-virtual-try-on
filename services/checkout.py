    ...
    *,
    ...
    for item in items:
        ...
        result = db.execute(
            ...
            .where(
                ProductVariant.id == variant_id,
                ProductVariant.stock_quantity - ProductVariant.reserved_quantity >= qty,
            )
            .values(reserved_quantity=ProductVariant.reserved_quantity + qty)
        )
        if result.rowcount != 1:
            db.rollback()
            raise InsufficientStockError(
                f"موجودی {variant.display_name} کافی نیست؛ لطفاً دوباره تلاش کنید."
            )
...
def complete_checkout(
    ...
    *,
    ...
    if not transition_allowed(checkout.state, "completed"):
