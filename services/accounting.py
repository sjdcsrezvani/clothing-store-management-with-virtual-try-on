...
from models import (
    Customer, Sale, SaleItem, Expense, Purchase, PurchaseItem, Payment,
)
...
def reverse_payment(db, payment) -> int:
    """Undo a نسیه payment: delete it and rebuild the customer's per-invoice
    allocation from the remaining payments (FIFO). Because the allocation is
    derived rather than stored, recomputing from the surviving payment rows is
    exact — debt, per-invoice paid amounts and settled flags all come back
    consistent. Returns the reversed amount."""
...
def get_cashbox(db, start, end, opening_balance: int) -> dict:
    """Cash register for a period: opening + cash in − cash out = closing."""
    cash_in_sales = db.query(func.coalesce(func.sum(Sale.final_amount), 0)).filter(
        Sale.payment_confirmed == True,
        Sale.is_refunded == False,
    ...
    cash_in_payments = db.query(func.coalesce(func.sum(Payment.amount), 0)).filter(
        Payment.method == "cash",
        Payment.created_at.between(start, end),
    ).scalar() or 0

    cash_out_refunds = db.query(func.coalesce(func.sum(Sale.refund_amount), 0)).filter(
        Sale.is_refunded == True,
        Sale.payment_method == "cash",
        Sale.refund_date.between(start, end),
    ).scalar() or 0

    cash_out_expenses = db.query(func.coalesce(func.sum(Expense.amount), 0)).filter(
    ...
    cash_out = cash_out_refunds + cash_out_expenses + cash_out_purchases
