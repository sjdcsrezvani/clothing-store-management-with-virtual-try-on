...
from models import (
    Customer, Sale, SaleItem, Expense, Purchase, PurchaseItem, Payment, Supplier, SupplierPayment,
)
...
def get_cashbox(db, start, end, opening_balance: int) -> dict:
    ...
    cash_out_supplier_payments = db.query(func.coalesce(func.sum(SupplierPayment.amount), 0)).filter(
